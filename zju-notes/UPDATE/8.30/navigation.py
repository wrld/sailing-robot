...
class Navigation(object):
    ...
    def __init__(self, 
        ...
        self.safety_zone_ll = safety_zone_ll
        self.safety_zone_margin = safety_zone_margin
        self.absolute_wind_direction_deque = deque(maxlen = MAXNUN)
        self.absolute_wind = 0.
        self.wind_direction = 0.
        self.wind_sum = 0
        self.wind_apparent = 0.
    ...
    def absolute_wind_direction(self):
        ...
        """
        # This assumes that our speed is negligible relative to wind speed.
        # rospy.logwarn("in navigation update absolute wind direction!!!")

        absolute_wind_direction = angleSum(self.heading, self.wind_direction)

        if len(self.absolute_wind_direction_deque) >= MAXNUN:
            self.wind_sum -= self.absolute_wind_direction_deque.popleft()

        self.wind_sum += absolute_wind_direction

        self.absolute_wind_direction_deque.append(absolute_wind_direction)

        self.absolute_wind = self.wind_sum / len(self.absolute_wind_direction_deque)
        
        self.wind_apparent = self.absolute_wind - self.heading

        # rospy.logwarn("self.wind_sum = " + str(self.wind_sum))
        # rospy.logwarn("len of deque = " + str(len(self.absolute_wind_direction_deque)))
        # rospy.logwarn("absolute_wind_direction = " + str(self.absolute_wind))

        return self.absolute_wind
