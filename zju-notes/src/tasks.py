...
class TasksRunner(object):
    def __init__(self, tasks, nav):
        ...
        self.taskdicts = tasks

        self.active_task_ll = ll()

        # rospy.logwarn('finish make tasks')
        self.check_jump_labels()

        self.pub = rospy.Publisher('active_task_ll', ll, queue_size=10)

        # rospy.logwarn('finish check_jump_labels')

    def check_jump_labels(self):
        '''
        check whether the jump point is specified, if not raise error
        '''
    ...
    def start_next_task(self):
        ...
        # rospy.logwarn('active_task = ' + str(self.active_task))
        # rospy.logwarn('task_kind = ' + str(self.active_task['kind']))
        rospy.logwarn('task_ll = ' + str(self.taskdicts[self.task_ix]['waypoint_ll']))
        active_task_ll = self.taskdicts[self.task_ix]['waypoint_ll']

        self.active_task_ll.lat = active_task_ll[0]
        self.active_task_ll.lon = active_task_ll[1]
    ...
    def calculate_state_and_goal(self):
        ...
        """
        # rospy.logwarn('begin calculate_state_and_goal')
        self.process_jump()
        # rospy.logwarn('task_id = ' + str(self.task_ix))
        # rospy.logwarn('check_end_condition = ' + str(self.active_task.check_end_condition()))
        ...
        self.pub.publish(self.active_task_ll)
